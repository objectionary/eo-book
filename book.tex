...
\begin{enumerate}
    ...
    \item (optional) The object may be declared as an atom (i.e., its implementation is made out of the EO language (for instance, in Java)) if the object is bound to a name (see \ref{item:2.3.1.3}). For this, the \ff{/} qualifier (see \ref{subsec:"/"-qualifier}) is used (for example, \ff{/bool}).
\end{enumerate}
...
\section{EO syntax}
%TODO: make an up-to-date version of the syntax
\begin{figure*}[ht]
  \centering
...
\subsection{EO}
%TODO: need to say more about it
...
\subsection{EO}
Doesn't exist and won't be introduced. The usual inheritance is presented by decorators (\ff{@} attribute). The main idea is that in the production (inheritance couses  many problems)
