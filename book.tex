...
Here, plus is an attribute of the object \ff{5} and \ff{times} is an attribute of the attribute object \ff{plus} (or, more precisely, an attribute of an object that \ff{plus} abstracts or dataizes to, which is an integer number \ff{int}).
...
\subsection{The \$ attribute}
The \ff{$} character is reserved for the special attribute self that every object has. The \ff{$} attribute is used to refer to the object itself.
The \ff{$} attribute may be useful to use the result of the object's dataization process for declaring other object's attributes.
The \ff{$} attribute may be used to access attributes of an object inside of the object with the dot notation (e.g., \ff{$.attrA}), but this notation is redundant.
...
[] > parentObject
  ...
  [] > childObject
    ...
    plus. > @
      ...
      magicNumber
\end{ffcode}
Here \ff{^.magicNumber} refers to the parent object's attribute and \ff{magicNumber} refers to \ff{^.magicNumber}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Containing a variable-length attribute (\ff{[animals...]}). The attribute must be at the end of the list of attributes to work properly. Internally, this attribute is represented by the array object.
    \end{enumerate}
    ...
    \item (\textit{optional}) The object may be declared as constant (i.e., dataized only once), if the object is bound to a name (see \ref{item:2.3.1.3}). For this, the \ff{!} operator is used.
    \item (optional) The object may be declared as an atom (i.e., its implementation is made out of the EO language (for instance, in Java)) if the object is bound to a name (see \ref{item:2.3.1.3}). For this, the \ff{/} operator is used (for example, \ff{/bool}).
\end{enumerate}

\subsection{Anonymous Abstraction}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item \textit{Containing one or more objects with names after each} (like \ff{1:a} \ff{5:b} \ff{9:c}). In this case, the listed objects will be bound to the corresponding free attributes of the applied object.
    \end{enumerate}
...
Now we can access all attributes of purchase and \ff{purchaseTotal} through a copy of purchaseTotal.
...
Attributes objects \ff{and}, \ff{or}, \ff{xor} are used for the bitwise operation of the same name. These attributes objects has one free attribute of type \ff{bytes} that is the second object (the first object is the base object of these attributes). Here is an example of using these attributes:
\begin{ffcode}
+package sandbox
+alias stdout org.eolang.io.stdout
+alias org.eolang.txt.sprintf
...
Attributes objects \ff{left}, \ff{right} are used for left and right bitwise shift. This attributes objects has one free attribute of type \ff{int} that is the second object (the first object is the base object of these attributes). This free attribute specifies how many bits to shift. Here is an example of using these attributes:
...
The \ff{bool} data type object represents a boolean value (either \ff{TRUE} or \ff{FALSE}) that can be used for performing logical operations.
...
\subsection{Syntax}
The \ff{bool} data type object may be parsed by the EO compiler directly from the source code. The syntax rules for \ff{bool} values are as follows. EBNF Notation:
\begin{ffcode}
BOOL     ::= 'TRUE'
           | 'FALSE'
\end{ffcode}
...
\ff{t} for the substitution if the base \ff{bool} object is \ff{TRUE}.
\ff{f} for the substitution if the base \ff{bool} object is \ff{FALSE}.
...
The \ff{float} data type object represents a double-precision 64-bit IEEE 754 floating-point number and can be used to perform various FPU computations.
...
\subsection{Syntax}
The \ff{float} data type object may be parsed by the EO compiler directly from the source code. The syntax rules for values are as follows. EBNF Notation:
...
The \ff{string} data type object represents a string literal.
...
\subsection{Syntax}
The \ff{string} data type object may be parsed by the EO compiler directly from the source code. The syntax rules for values are as follows. EBNF Notatio:
\begin{ffcode}
...
The object \ff{org.eolang.txt.text} is a decorator of \ff{org.eolang.string}. So the \ff{text} object contains methods for working with strings. Before using these methods, you need to make such an application:
\begin{ffcode}
...
The \ff{int} data type object represents a 64-bit integer number.
Fully Qualified Name: \ff{org.eolang.int} (no aliasing or FQN reference required since the object is automatically imported).

\subsection{Syntax}
The \ff{int} data type object may be parsed by the EO compiler directly from the source code. The syntax rules for values are as follows. EBNF Notation:
...
The \ff{lt} attribute object is used for testing if its base int object is less than its \ff{x} free attribute (i.e. \ff{$ < x}).
If the less attribute object is fully applied, it represents the result of the testing (either \ff{TRUE} (if the base object is less than \ff{x} free attribute of the less) or \ff{FALSE} (otherwise)). Here is an example of using \ff{lt} attribute:
...
The object \ff{org.eolang.math.number} is a decorator of \ff{org.eolang.int}. So the number object contains methods for working with \ff{int} data type objects. Before using these methods, you need to make such an application:
\begin{ffcode}
...
The object \ff{org.eolang.collections.list} is a decorator of \ff{org.eolang.array}. So the \ff{list} object contains methods for working with arrays. Before using these methods, you need to make such an application:
\begin{ffcode}
...
%TODO: potential formalizability of semantics
